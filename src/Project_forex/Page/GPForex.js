...
function forex_stream(obj, name, sett) {
  if (obj.stream_debug == 'Popy') {
    return <Box sx={{ display: 'flex' }} style={{ position: "relative", left: "20px" }} >
      <h6 style={{color:"#646464"}}>Londing...</h6><CircularProgress size="1.3rem"  style={{marginLeft:"10px"}}/>
    </Box>
  }
  else {
    if (sett == 'BID') {
      try {
        if (obj.stream[name]["stream"][0] >= obj.stream[name]["previous"][0]) {
          return <h4 style={{ color: "green", display: "inline-block", marginLeft: "10px", fontSize: "1.1em" }} >&nbsp;&nbsp;{obj.stream[name]["stream"][0]}</h4>
        }
        else {
          return <h4 style={{ color: "red", display: "inline-block", marginLeft: "10px", fontSize: "1.1em" }} >&nbsp;&nbsp;{obj.stream[name]["stream"][0]}</h4>
        }
        //return <h4 style="color:red" >&nbsp;&nbsp;{obj.stream[name][0]}</h4>
      } catch (e) {
        console.log("stream error ", obj.stream[name])
        return <h4 style={{ display: "inline-block", marginLeft: "10px", fontSize: "1.1em" }}>Market close { console.log("Market close")}</h4>
      }
    }
    else //sett == 'ASK'
    {
      try {
        if (obj.stream[name]["stream"][1] >= obj.stream[name]["previous"][1]) {
          return <h4 style={{ color: "green", display: "inline-block", marginLeft: "10px", fontSize: "1.1em" }} >&nbsp;&nbsp;{obj.stream[name]["stream"][1]}</h4>
        }
        else {
          return <h4 style={{ color: "red", display: "inline-block", marginLeft: "10px", fontSize: "1.1em" }} >&nbsp;&nbsp;{obj.stream[name]["stream"][1]}</h4>
        }
...
function hist_gra_handle(obj, name, timeframe) // wrapper for showing charts to catch if there is no chart data had been fetch yet(witch it usually will) and we'll handle that by show loading text
{
  ...
  try {
    ...
    return <Box sx={{ display: 'flex' }} style={{ position: "relative", left: "20px" }} >
            <h5 style={{color:"#646464"}}>Londing.....</h5><CircularProgress size="1.3rem" style={{marginLeft:"20px"}} />
    </Box>
  }
}
//*
                ...
                <div className='BG-zigzag '>
                  ...
                  <Grid container style={{ position: "relative", top: "7px" }}>
                    ...
                    <Grid item xs={6} sm={4} md={4} lg={4} style={{ textAlign: "center" }} >
                      <h4 className='font-BidandAsk'  ><span style={{ fontSize: "1.1em" }}>BID</span><span className='Colortext-Priceandmore'>{forex_stream(stream, forex_name, 'BID')}</span></h4>
                    </Grid>
                    <Grid item xs={6} sm={4} md={4} lg={4} style={{ textAlign: "center" }}>
                      <h4 className='font-BidandAsk' ><span style={{ fontSize: "1.1em" }}>ASK</span><span className='Colortext-Priceandmore'>{forex_stream(stream, forex_name, 'ASK')}</span></h4>
                    </Grid>

                    <Grid item xs={12} sm={4} md={4} lg={4}>
                      <Link to={ENDPOINT} style={{ textDecoration: "none", position: "relative", top: "-7px" }}>
                        <div className='position-buttuon'>
                          <Button variant="contained" style={{ width: "150px", height: "40px" }}>ดูสถิติการกลับตัว</Button>
                        </div>
                      </Link>
                    </Grid>
