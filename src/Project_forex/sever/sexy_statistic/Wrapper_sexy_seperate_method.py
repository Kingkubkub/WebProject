...
def run(symbol, timeframe, from_date, to_date, depth, diviation, backstep):
    with open('GC.csv', 'rb') as rawdata: # this use to see what unicode is to put in a pd.read_csv or else if you don't put correct unicode you end up get nothing from your data but error or wrong encode data
    ...
    ##### 1. READ DATA
    data = pd.read_csv('GC.csv', encoding='ascii', header=None, sep="\t") #this is test sample in real sample  there will be thing we don't need   #**Important I use sep="\t" wich \t not exist in real data cause of evade some error
    #example of sample 
    ...
    json_initial =  "{ \""+symbol+"\":\"waiting to be redeclare\" }"
    json_initial = json.loads(json_initial)
    json_initial[symbol] = {} #from here you can add any value you want like a breads and butter if you not add {} first there will be one value it can hold
    json_initial[symbol]["name"] = symbol ##^ ["time_freme"] = timeframe
    json_initial[symbol][timeframe_for_json]={}
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]={}
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["depth"]=depth
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["diviation"]=diviation
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["backstep"]=backstep
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_date"]=from_date
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["to_date"]=to_date
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Hight_to_Low_aka_GC"] = {} ###^ 
    json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Low_To_Hight_aka_GC"] = {} ###^ 
    ...
    for i in range(min_lenght_of_direction_one_and_two): #start from 0 like nomal index in array
        ...
        try:## JSON it self** +1 to the value that "from_high" went to "to_low" IF it aready exist if not create one with the name of to_low_xxx where xxx is how far from_high went to and set value to 1 ***Mean from_high went to this to_low "+1" time(s)
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Hight_to_Low_aka_GC"][from_high_]
        except KeyError:
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Hight_to_Low_aka_GC"][from_high_] = {}

        try:
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Hight_to_Low_aka_GC"][from_high_][to_low_] += 1
        except KeyError:
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Hight_to_Low_aka_GC"][from_high_][to_low_] = 1
    #*
    ####*
    ...
    for i in range(min_lenght_of_direction_one_and_two): #start from 0 like nomal index in array
        ...
        try:## JSON it self** +1 to the value that "from_high" went to "to_low" IF it aready exist if not create one with the name of to_low_xxx where xxx is how far from_high went to and set value to 1 ***Mean from_high went to this to_low "+1" time(s)
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Low_To_Hight_aka_GC"][from_low_]
        except KeyError:
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Low_To_Hight_aka_GC"][from_low_] = {}

        try:
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Low_To_Hight_aka_GC"][from_low_][to_high_] += 1
        except KeyError:
            json_initial[symbol][timeframe_for_json][zigzag_param_for_json]["from_Low_To_Hight_aka_GC"][from_low_][to_high_] = 1
        #*
    ####*
...
run("EURUSD","M5","2022.01.01","2022.02.02", "60", "30", "15") #test here